...
// onChange={(e) => purchase.setOrder.credit_card_number(e.target.value)}


/*
const location = useLocation();
    console.log(location)
    const handleSubmit = (e) => { navigate('/purchase/paymentEntry/viewOrder', { state: {order: location.state.order} })};
    const navigate = useNavigate();

    return(
    
        <div> 
            {location.state.order.buyQuantity[2]} 
            <h1>
                ...
                <form onSubmit = {handleSubmit}>
                    ...
                    <input>
                        type="text"
                        required
                        onChange{(e) => {{location.state.order.credit_card_number = e.target.value;}}}
                    </input>
                </form>
        </div>
...
const PaymentEntry = () => {
    ...
    const handleSubmit = (e) => {
        navigate('/purchase/paymentEntry/viewOrder', {state:{order: location.state.order}}
        )
    };
